...
type Downloader interface {
	...
	PinAsset(uri string) (assets.PublishedAsset, error)
}
...
var extMap = map[Spotify.AudioFile_Format]string{
	...
	Spotify.AudioFile_AAC_160:        ".160.aac",
	Spotify.AudioFile_AAC_320:        ".320.aac",
}
...
func (dl *downloader) PinAsset(assetURI string) (assets.PublishedAsset, error) {

	if len(assetURI) != 22 {
		panic("TODO: add URI parsing")
	}

	assetID := utils.Base62ToHex(assetURI)
	...
	for _, fi := range track.GetFile() {
		if fi.GetFormat() == dl.audioFormat {
			trackFile = fi
			break
		}
	}
